...
unless(@ARGV>=4){
	print "Usage: you should input more than three args
 	perl $0 dmrDir samNumber statistics length fold diff p_threshold point_threshold dmrfile\n";
	exit;
}
my $dmrdir=$ARGV[0];
my $sam_num=$ARGV[1];
...
open DMROUT,">$dmrfile.Extend"||die"$!";
...
#### if all condition is ok, keep going; if not, print the before DMR ####
				...
				if($points_bak>=$points_threshold){
					...
					#print "222222222\t$temp_join_bak[0]\n333333333\t$temp_join_bak[1]\n";
					$pval=&{$statistics}(\@input_bak);
...
`sort -k1,1 -k2,2n $dmrfile.Extend -o $dmrfile.Extend`;
